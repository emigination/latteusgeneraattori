...
class TestSanalaskija(unittest.TestCase):
    ...
    def test_lue_opetusdata(self):
        self.assertEqual(self.sanalaskija._lue_opetusdatatiedosto(self.opetusdata.name), ['Tämä on tekstiä.', 'Vähän lisää tekstiä.', 'Lorem ipsum dolor sit amet. Consectetur adipiscing elit.'])

    def test_todennakoisyydet_oikein(self):
        seuraavat = [[1, 2], [2, 2, 0, 2], [1]]
        self.assertEqual(self.sanalaskija._laske_todennakoisyydet(
            seuraavat, 3), [[0, 0.5, 0.5], [0.25, 0, 0.75], [0, 1, 0]])
