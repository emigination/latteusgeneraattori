...
class TestSanalaskija(unittest.TestCase):
    ...
    def test_seuraavien_sanojen_lista(self):
        seuraavaat = self.sanalaskija._laske_sanat(['Tämä on tekstiä.', 'Niin on tämäkin.', 'Lorem ipsum dolor sit amet.'])
        self.assertEqual(seuraavaat, [[1], [2,4], [], [1], [], [6], [7], [8], [9], []])

    def test_todennakoisyydet_oikein(self):
        todennakoisyydet = self.sanalaskija._laske_todennakoisyydet([[1, 2], [2, 2, 0, 2], [1]], 3)
        self.assertEqual(todennakoisyydet, [[0, 0.5, 0.5], [0.25, 0, 0.75], [0, 1, 0]])
